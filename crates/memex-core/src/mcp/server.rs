...
fn handle_request(req: &JsonRpcRequest, db: &Db) -> JsonRpcResponse {
    match req.method.as_str() {
        ...
        "notifications/initialized" => return JsonRpcResponse::success(req.id.clone(), serde_json::json!({})),
...
fn handle_tool_call(req: &JsonRpcRequest, db: &Db) -> JsonRpcResponse {
    ...
    match result {
        Ok(content) => JsonRpcResponse::success(req.id.clone(), serde_json::json!({
            "content": [{ "type": "text", "text": content }]
        })),
        Err(msg) => JsonRpcResponse::success(req.id.clone(), serde_json::json!({
...
fn tool_search(db: &Db, args: &serde_json::Value) -> std::result::Result<String, String> {
    ...
    let filter = SearchFilter {
        adapter: args.get("adapter").and_then(|v| v.as_str()).map(String::from),
        project: args.get("project").and_then(|v| v.as_str()).map(String::from),
        ..Default::default()
    };

    let retriever = Retriever::new(db);
    let results = retriever.search_filtered(query, limit, &filter).map_err(|e| e.to_string())?;
...
fn tool_get_session(db: &Db, args: &serde_json::Value) -> std::result::Result<String, String> {
    ...
    let resolved = if sid.len() < 36 {
        db.find_session_by_prefix(sid).map_err(|e| e.to_string())?
            .unwrap_or_else(|| sid.to_string())
    } else {
        sid.to_string()
    };
    let detail = db.get_session_detail(&resolved).map_err(|e| e.to_string())?;
    match detail {
...
fn tool_list_recent(db: &Db, args: &serde_json::Value) -> std::result::Result<String, String> {
    ...
    let sessions = db.list_sessions(limit).map_err(|e| e.to_string())?;
