...
impl Db {
    ...
    pub fn workload_report(&self, days: u32) -> Result<WorkloadReport> {
        ...
        // 0) 每日明细 — 日历视图原料。
        //    sessions = 今天 updated_at 落在该天的 session 数（不变）。
        //    messages = 两条通路 union 后 SUM：
        //      - msg_by_day_a：有 timestamp 的 messages 按真实时间 COUNT(*)；
        //      - msg_by_day_b：没 timestamp 的 session，按 updated_at 落桶时
        ...
        // 4) 整体汇总。messages 与 daily/heatmap 同源用两条数据通路相加，
        //    避免 SUM(sessions.message_count) 把跨天长会话生命周期消息全
        //    算到「窗口起点之后」一次的反复计入。
        let (sessions, messages, active_days, peak_day, peak_day_sessions): (
            ...
            // 通路 A：有 timestamp 的 messages 直接 COUNT
            let total_messages_a: i64 = conn
                .query_row(
            ...
            // 通路 B：没 timestamp 的 session，按 session.message_count 求和
            let total_messages_b: i64 = conn
                .query_row(
            ...
            let total: (i64, i64) = (total_sessions, total_messages_a + total_messages_b);
