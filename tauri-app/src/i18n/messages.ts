...
const zh: Messages = {
  ...
  'settings.llm.ollama_missing.claude_fallback_hint': '已启用 Claude 兜底：未装 Ollama 时会自动改走 Claude API 完成上面的功能（需要在凭证里配 API Key）。',
  'settings.llm.ollama_missing.claude_fallback_suggest': '提示：如果不想装 Ollama，可在下方启用 Claude 兜底并配置 API Key 来作为替代。',
...
const en: Messages = {
  ...
  'settings.llm.ollama_missing.claude_fallback_hint': 'Claude fallback is on: Memex will use the Claude API for the above features when Ollama is unavailable (requires API key in credentials).',
  'settings.llm.ollama_missing.claude_fallback_suggest': 'Tip: if you don\'t want to install Ollama, you can enable Claude fallback below and configure an API key as an alternative.',
