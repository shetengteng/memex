...
> **2026-06-02 更新**：移除了 Tauri Updater。原因：app 体积小、用户能用 `brew upgrade` 升级，
> 维护 signing key 收益不匹配。所以 **release.yml 不再需要任何 GitHub Secret**。
> （对比：tt-conduit 同模式，更简洁。）

### Phase 0：一次性准备（每个 GitHub 账号只做一次）

#### 0.1 创建 Homebrew tap repo
...
2. **release job**：在 ubuntu runner 上：
   - 下载两组 artifact
   - 用 `softprops/action-gh-release@v2` 创建 GitHub Release，上传 DMG
...
### O3：（已剥离）Tauri Updater

曾经计划用 Tauri Updater 实现 in-app 升级，但维护 signing key + GHA secret 与
带来的价值不匹配。已于 2026-06-02 移除。如果以后要恢复，参考 commit 历史。
...
| 阻塞项 | 阻塞什么 | 解决方法 |
|---|---|---|
| 没创建 `shetengteng/homebrew-memex` repo | Phase 3 无法 push cask | Phase 0.1 |
